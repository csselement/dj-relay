...
When the broadcast ends, its original listener link and any shared listener links remain available until the producer deletes the session. Unrecorded links show that the session has concluded; recorded links continue into private replay access. Replay viewers can also copy a durable session link from the replay card. The finalizer includes every physical MediaMTX segment in one 192 kbps stereo MP3, then verifies codec, channels, sample rate, duration, a full-file decode, and a SHA-256 digest before publishing it. A recording is not marked ready unless those checks pass.

Original Opus/fMP4 segments remain untouched throughout transcoding and validation. Discus deletes them automatically only after the complete MP3 passes every validation check, the validation manifest is atomically published, and the recorded source-segment list is confirmed to be fully represented. Failed validation, interrupted work, or a source mismatch preserves every original segment for retry. Explicit producer deletion removes any remaining source segments together with the MP3 archive.

The `relay-recordings` and `relay-playback` Docker volumes store original and finalized media separately from SQLite. Original segments persist across deployments and retries until validated finalization removes them; finalized MP3s remain until producer deletion. Neither volume is included in the SQLite backup or copied off-device. MediaMTX playback on port 9996 is reachable only inside the Docker network; the application authorizes and proxies every replay request.
...
Legacy on-demand conversion remains capacity-limited to two active and four queued FFmpeg jobs. New archives use the background finalizer and serve only the stored MP3 after its validation manifest has been written.
