...
Original Opus/fMP4 segments are never removed by finalization, successful retries, application restarts, or MP3 playback. They remain alongside the validated MP3 until the producer explicitly deletes the archived session, at which point Discus removes both representations. This provides a recoverable source even if a future MP3 needs to be regenerated.

The `relay-recordings` and `relay-playback` Docker volumes store original and finalized media separately from SQLite and are retained across deployments until a producer deletes the session. They are not included in the SQLite backup or copied off-device. MediaMTX playback on port 9996 is reachable only inside the Docker network; the application authorizes and proxies every replay request.
